...
if [[ ! -t 0 ]]; then
  ...
  | fzf -d ':' -n 2.. --ansi --reverse --keep-right --multi \
    ...
    --bind="alt-l:execute-silent(echo {+} | ~/.bin/f_a_txt_to_url_open)+accept,alt-t:execute-silent(echo {+} | ~/.bin/f_a_txt_to_url_open_in_safari_tabs)+accept,ctrl-x:execute-silent(~/.bin/print_args {+} | ~/.bin/safecopy)+accept,ctrl-u:preview-page-up,ctrl-d:preview-page-down" \
