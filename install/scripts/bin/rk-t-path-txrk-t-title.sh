...
while read -r filename; do
  ...
  if [[ $filename_no_ext =~ ^[a-z0-9_-]*$ ]]; then
    ...
    title=$(echo "$filename_no_ext" | perl -pe 's/([0-9]+?)-([^0-9]+?)/\1 \2/g' | perl -pe 's/([^0-9]+?)-([0-9]+?)/\1 \2/g' | perl -pe 's/([^0-9]+?)-([^0-9]+?)/\1 \2/g' | tr "_" " " | ~/.bin/rk-f-titlecase)
  else
    title=$(echo "$filename_no_ext" | ~/.bin/rk-f-titlecase)
  fi
  echo "$title"
done
