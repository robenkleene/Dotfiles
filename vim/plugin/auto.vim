...
augroup safecopy
  ...
  autocmd TextYankPost * if v:event["regname"] !=# '*' && v:event["regname"] !=# '+' && v:event["operator"] =~# '^[yd]$' && &ft !=# "netrw" | call system(has('gui_running') ? 'pbcopy' : '~/.bin/nobin/_safecopy.sh',join(v:event["regcontents"],"\n")) | end
augroup END
